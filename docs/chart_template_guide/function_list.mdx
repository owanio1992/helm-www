...
Helm has the following gzip compression and decompression functions:

- `gzip`: Compress a string with gzip and return as a Base64 encoded string
- `ungzip`: Decode a Base64 encoded string and decompress with gzip
...
It enforces a size limit of 1MB on both the input string and the decompressed output. It also verifies that the decompressed content is valid UTF-8 text to prevent non-textual data from being obfuscated.
